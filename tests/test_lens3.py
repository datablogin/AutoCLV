...
class TestLens3Metrics:
    ...
    def test_unordered_periods_raises_error(self):
        ...
        periods = [
            CohortPeriodMetrics(1, 80, 0.8, 1.2, 40.0, 1.2, 40.0, 3200.0),
            CohortPeriodMetrics(0, 100, 1.0, 1.5, 50.0, 1.5, 50.0, 5000.0),  # Out of order
        ]
        with pytest.raises(ValueError, match="periods must be ordered by period_number"):
            Lens3Metrics(
                cohort_name="2023-01",
                acquisition_date=datetime(2023, 1, 1),
                cohort_size=100,
    ...
    def test_non_contiguous_period_numbers_raises_error(self):
        ...
        periods = [
            CohortPeriodMetrics(0, 100, 1.0, 1.5, 50.0, 1.5, 50.0, 5000.0),
            CohortPeriodMetrics(2, 60, 0.6, 1.0, 35.0, 1.0, 35.0, 2100.0),  # Skips period 1
        ]
...
class TestAnalyzeCohortEvolution:
    ...
    def test_simple_cohort_evolution(self):
        ...
        period_aggregations = [
            ...
            PeriodAggregation("C3", datetime(2023, 1, 1), datetime(2023, 2, 1), 1, 75.0, 20.0, 3),
            # Period 1: Only C1 and C3 active (C2 churned)
            PeriodAggregation("C1", datetime(2023, 2, 1), datetime(2023, 3, 1), 1, 75.0, 20.0, 3),
            PeriodAggregation("C3", datetime(2023, 2, 1), datetime(2023, 3, 1), 2, 100.0, 30.0, 4),
            # Period 2: Only C1 active
            PeriodAggregation("C1", datetime(2023, 3, 1), datetime(2023, 4, 1), 1, 50.0, 15.0, 2),
        ]
    ...
    def test_cohort_with_single_period(self):
        ...
        period_aggregations = [
            PeriodAggregation("C1", datetime(2023, 1, 1), datetime(2023, 2, 1), 1, 100.0, 30.0, 5),
            PeriodAggregation("C2", datetime(2023, 1, 1), datetime(2023, 2, 1), 1, 50.0, 15.0, 2),
        ]
    ...
    def test_cohort_with_100_percent_retention(self):
        ...
        period_aggregations = [
            ...
            # Period 1
            PeriodAggregation("C1", datetime(2023, 2, 1), datetime(2023, 3, 1), 1, 75.0, 20.0, 3),
            PeriodAggregation("C2", datetime(2023, 2, 1), datetime(2023, 3, 1), 1, 60.0, 18.0, 3),
        ]
    ...
    def test_no_matching_period_aggregations_raises_error(self):
        ...
        cohort_customers = ["C1", "C2"]
        period_aggregations = [
            PeriodAggregation("C99", datetime(2023, 1, 1), datetime(2023, 2, 1), 1, 100.0, 30.0, 5),
        ]
    ...
    def test_acquisition_date_after_all_periods_raises_error(self):
        ...
        with pytest.raises(ValueError, match="No periods found on or after acquisition_date"):
            analyze_cohort_evolution(
                cohort_name="2023-01",
    ...
    def test_avg_metrics_calculations(self):
        ...
        period_aggregations = [
            ...
            PeriodAggregation("C1", datetime(2023, 1, 1), datetime(2023, 2, 1), 3, 300.0, 90.0, 10),
            # C2: 1 order, $100 revenue
            PeriodAggregation("C2", datetime(2023, 1, 1), datetime(2023, 2, 1), 1, 100.0, 30.0, 5),
        ]
        ...
        assert metrics.periods[0].active_customers == 2
        assert metrics.periods[0].avg_orders_per_customer == 2.0  # 4 orders / 2 customers
        assert metrics.periods[0].avg_revenue_per_customer == 200.0  # $400 / 2 customers
    ...
    def test_filters_out_non_cohort_customers(self):
        ...
        period_aggregations = [
            ...
            PeriodAggregation("C99", datetime(2023, 1, 1), datetime(2023, 2, 1), 5, 500.0, 150.0, 20),
        ]
    ...
    def test_retention_is_monotonically_non_decreasing(self):
        ...
        period_aggregations = [
            ...
            PeriodAggregation("C1", datetime(2023, 1, 1), datetime(2023, 2, 1), 1, 100.0, 30.0, 5),
            PeriodAggregation("C2", datetime(2023, 1, 1), datetime(2023, 2, 1), 1, 100.0, 30.0, 5),
            PeriodAggregation("C3", datetime(2023, 1, 1), datetime(2023, 2, 1), 1, 100.0, 30.0, 5),
            # Period 1: Only C1 active (retention stays 100% - C2, C3 already counted)
            PeriodAggregation("C1", datetime(2023, 2, 1), datetime(2023, 3, 1), 1, 100.0, 30.0, 5),
            # Period 2: C1 and C2 return (retention stays 100%)
            PeriodAggregation("C1", datetime(2023, 3, 1), datetime(2023, 4, 1), 1, 100.0, 30.0, 5),
            PeriodAggregation("C2", datetime(2023, 3, 1), datetime(2023, 4, 1), 1, 100.0, 30.0, 5),
            # Period 3: Only C3 active (retention stays 100% - all customers seen)
            PeriodAggregation("C3", datetime(2023, 4, 1), datetime(2023, 5, 1), 1, 100.0, 30.0, 5),
        ]
