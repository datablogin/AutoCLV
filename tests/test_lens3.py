...
class TestLens3Metrics:
    ...
    def test_unordered_periods_raises_error(self):
        ...
        periods = [
            CohortPeriodMetrics(1, 80, 0.8, 1.2, 40.0, 3200.0),
            CohortPeriodMetrics(0, 100, 1.0, 1.5, 50.0, 5000.0),  # Out of order
        ]
    ...
    def test_non_contiguous_period_numbers_raises_error(self):
        ...
        periods = [
            CohortPeriodMetrics(0, 100, 1.0, 1.5, 50.0, 5000.0),
            CohortPeriodMetrics(2, 60, 0.6, 1.0, 35.0, 2100.0),  # Skips period 1
        ]
...
class TestAnalyzeCohortEvolution:
    ...
    def test_simple_cohort_evolution(self):
        ...
        # Period 1: 2 customers (66.67% retention)
        assert metrics.periods[1].period_number == 1
        assert metrics.periods[1].active_customers == 2
        assert metrics.periods[1].retention_rate == pytest.approx(2 / 3, rel=1e-9)
        assert metrics.periods[1].total_revenue == 175.0  # 75 + 100

        # Period 2: 1 customer (33.33% retention)
        assert metrics.periods[2].period_number == 2
        assert metrics.periods[2].active_customers == 1
        assert metrics.periods[2].retention_rate == pytest.approx(1 / 3, rel=1e-9)
...
class TestCalculateRetentionCurve:
    ...
    def test_single_period_returns_single_point_curve(self):
        ...
        periods = [
            CohortPeriodMetrics(0, 50, 1.0, 2.0, 60.0, 3000.0),
        ]
        metrics = Lens3Metrics(
            cohort_name="2023-Q1",
            acquisition_date=datetime(2023, 1, 1),
